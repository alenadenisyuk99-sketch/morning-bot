...
def gramotey(text):
    return claude(f"""Ти — літературний редактор. Виправ усі мовні помилки в тексті.

ЩО ВИПРАВЛЯТИ:
1. Русизми та суржик: "да"→"так", "нет"→"ні", "ладно"→"добре", "конечно"→"звісно", "очень"→"дуже", "вообще"→"взагалі", "мелочей"→"дрібниць", "траты"→"витрати", "дверь"→"двері".
2. Вигадані або незрозумілі слова — переписати простіше.
3. Помилки відмінювання і правопису.
4. Штучні кальки з російської/англійської — переписати живою мовою.

ПРАВИЛО: краще просто і зрозуміло, ніж "красиво" але дивно.
...
Правила:
— Пиши так, як українець говорить з друзями — просто, природно, без пафосу.
...
— Конкретно і по-людськи: "зателефонуй мамі сьогодні" — добре. "Зірки сприяють гармонії" — погано.
