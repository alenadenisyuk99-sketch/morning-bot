...
# Assign a different life sphere to each sign daily (rotates by day)
SPHERES = [
    "кохання та стосунки",
    "фінанси та гроші",
    "кар'єра та амбіції",
...
sign_lines = "\n".join([
    f"{emoji} {name} [сфера дня: {sphere}]: ..."
    for (emoji, name), sphere in zip(signs, daily_spheres)
])

horoscope_text = claude(f"""Ти — астролог, який щодня пише гороскоп для команди живих людей у Telegram-чаті. Сьогодні {today.strftime("%d.%m.%Y")}.

Правила написання:
— Мова: виключно грамотна літературна українська. Жодних російських слів, суржику, кальок з російської.
— Стиль: теплий, живий, трохи з гумором — як пише популярний астролог в Instagram. Не канцелярит, не повчання.
— Обсяг: рівно 2 речення на кожен знак.
— Різноманітність: кожен знак сьогодні отримує підказку в іншій сфері життя (вказано нижче). Не прив'язуйся до стереотипів знаку — сьогодні Лев може чути про гроші, а Риби — про кар'єру.
— Конкретність: давай реальну пораду або спостереження, а не розмиті "зірки сприяють успіху".
— Тон: один знак — про любов, інший — про фінанси, третій — про сім'ю. Нехай люди щоразу дізнаються щось нове про свій день.

Починай повідомлення з: Доброго ранку, команда! Зорі сьогодні кажуть:

{sign_lines}

Заповни кожен знак двома реченнями відповідно до його сфери дня.""")
...
Напиши 3 речення грамотною літературною українською мовою (без суржику та русизмів) — тепло і з легким гумором, для живих людей що починають свій день.
